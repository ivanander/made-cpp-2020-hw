...
namespace task {
    ...
    VectorDouble operator+(const VectorDouble& lhs, const VectorDouble& rhs) {
        auto res = lhs;
        std::transform(begin(lhs), end(lhs), begin(rhs), begin(res), std::plus<double>{});
        return res;
    }

    VectorDouble operator+(const VectorDouble& lhs) {
        auto res = lhs;
        std::transform(begin(lhs), end(lhs), begin(res), [] (double val) { return +val; });
        return res;
    }
    ...
    VectorDouble operator-(const VectorDouble& lhs, const VectorDouble& rhs) {
        auto res = lhs;
        std::transform(begin(lhs), end(lhs), begin(rhs), begin(res), std::minus<double>{});
        return res;
    }

    double operator*(const VectorDouble& lhs, const VectorDouble& rhs) {
        auto res = lhs;
        std::transform(begin(lhs), end(lhs), begin(rhs), begin(res), std::multiplies<double>{});
    ...
    VectorDouble operator%(const VectorDouble& lhs, const VectorDouble& rhs) {  // only for 3-dimensional vectors
        auto res = lhs;
    ...
    VectorInt operator|(const VectorInt& lhs, const VectorInt& rhs) {
        auto res = lhs;
        std::transform(begin(lhs), end(lhs), begin(rhs), begin(res), [] (int lhs, int rhs) { return lhs | rhs; });
        return res;
    }
